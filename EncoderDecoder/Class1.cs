...
namespace EncoderDecoder
{
    ...
    public class Encoder : IEncoder
    {
        ...
        public byte[] Encode(DataPackage inputPackage)
        {
            ...
            _array[13] = velocityGet(inputPackage.DataF)[1];//获取速度
            _array[14] = velocityGet(inputPackage.DataF)[0];
        ...
        private byte[] velocityGet(double dataF) //获得速度
        {
            ...
            _velocity[0] = Convert.ToByte((int)dataF);
            _velocity[1] = Convert.ToByte((int)dataF >> 8);
            return _velocity;
        }
        ...
        private byte[] xiaoshutiqu(double dataX)
        {
			...
			int _intDecimal = (int)(_decimal * 1000);//变成整数

			_decimalData[0] = Convert.ToByte((_intDecimal - (int)(_intDecimal / 100) * 100));
			_decimalData[1] = Convert.ToByte((_decimal - _decimalData[0]) / 100);
    ...
    {        
        ...
        {
			byte[] buf = new byte[2];
			buf[0] = inputPackage[9];
			buf[1] = inputPackage[10];
			int tempInt = BitConverter.ToInt16(buf, 0);

			double tempDec = (inputPackage[11] * 100 + inputPackage[12]) / 10000;
        ...
        {
			byte[] buf = new byte[2];
			buf[0] = inputPackage[5];
			buf[1] = inputPackage[6];
			int tempInt = BitConverter.ToInt16(buf, 0);

			double tempDec = (inputPackage[7] * 100 + inputPackage[8]) / 10000;
        ...
        {
			byte[] buf = new byte[2];
			buf[0] = inputPackage[1];
			buf[1] = inputPackage[2];
			int tempInt = BitConverter.ToInt16(buf, 0);

			double tempDec = (inputPackage[3] * 100 + inputPackage[4]) / 10000;
