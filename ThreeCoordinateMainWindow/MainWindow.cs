...
namespace ThreeCoordinateMainWindow
{
	public partial class MainWindow : Form
	{
		...
		static AutoResetEvent _displayProgress;//控制进度显示线程

		bool _isControlThreadSuspended;//控制线程是否挂起的标志
		//bool _isTableFinished;//工作台是不是完成了上一条指令
		...
		double _gWorkStep = 0.001;//G代码插补步长，单位毫米
		...
		private void MainWindow_Load(object sender, EventArgs e)
		{
			...
			_displayProgress = new AutoResetEvent(false);
		...
		private void 打开ToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			{
				Thread _progressWindow = new Thread(new ThreadStart(progressControlThread));

				_progressWindow.Start();

				_displayProgress.WaitOne();
		...
		private void MainWindow_KeyDown(object sender, KeyEventArgs e)
		{
			if(ckbIsKeyControl.Checked == true)
			{
				...
				{
					tempPackage.DataX = 0;
					tempPackage.DataY = _keyControlMul;
					tempPackage.DataZ = 0;
					tempPackage.DataF = 0;
				...
				{
					tempPackage.DataX = 0;
					tempPackage.DataY = -1 * _keyControlMul;
					tempPackage.DataZ = 0;
					tempPackage.DataF = 0;
					tempPackage.Command = 0;
				}
				...
				{
					tempPackage.DataX = -1 * _keyControlMul;
					tempPackage.DataY = 0;
					tempPackage.DataZ = 0;
					tempPackage.DataF = 0;
					tempPackage.Command = 0;
				...
				{
					tempPackage.DataX = _keyControlMul;
					tempPackage.DataY = 0;
					tempPackage.DataZ = 0;
					tempPackage.DataF = 0;
					tempPackage.Command = 0;
				...
				else if (e.KeyCode == Keys.W)
				{
					tempPackage.DataX = 0;
					tempPackage.DataY = 0;
					tempPackage.DataZ = _keyControlMul;
					tempPackage.DataF = 0;
					tempPackage.Command = 0;
				}
				else if (e.KeyCode == Keys.S)
				{
					tempPackage.DataX = 0;
					tempPackage.DataY = 0;
					tempPackage.DataZ = -1 * _keyControlMul;
					tempPackage.DataF = 0;
					tempPackage.Command = 0;
				}
				else
		...
		public void areaWheel_MouseWheel(object sender, MouseEventArgs e)
		{
			...
			if (e.Delta > 0)//上滚
			{
				...
				switch (coordinate)
				{
					case 1: tempPackage.DataX = temp * _gWorkStep;break;
					case 2: tempPackage.DataY = temp * _gWorkStep;break;
					case 3: tempPackage.DataZ = temp * _gWorkStep;break;
				}
			}
			else if (e.Delta < 0)//下滚
			{
				switch (coordinate)
				{
					case 1: tempPackage.DataX = -1* temp * _gWorkStep; break;
					case 2: tempPackage.DataY = -1* temp * _gWorkStep; break;
					case 3: tempPackage.DataZ = -1* temp * _gWorkStep; break;
				}
			}
			...
			else
			{
				foreach (GCodeClass oneLine in GCodeInterpreted)//取每一个G代码编译的指令
				{
					...
					if (oneLine.GWorkState == GWorkState.G00)//G00指令
					{
						...
						tempPackage.DataX = (targetRelative + _deltaCoordinate).X;//传送X绝对坐标
						tempPackage.DataY = (targetRelative + _deltaCoordinate).Y;//传送X绝对坐标
						tempPackage.DataZ = (targetRelative + _deltaCoordinate).Z;//传送X绝对坐标
						tempPackage.DataF = 0;
					...
					else if (oneLine.GWorkState == GWorkState.G01)//G01指令
					{
						...
						foreach (StepMark SM in tempMark)
						{
							...
							{
								tempPackage.DataF = targetF;
								tempPackage.DataX = (targetRelative - _relativeCoordinate).X;
								tempPackage.DataY = 0;
								tempPackage.DataZ = 0;
								tempPackage.Command = 0;
							}
							else if (SM.Y == StepState.ToEnd)//y到头
							{
								tempPackage.DataF = targetF;
								tempPackage.DataX = 0;
								tempPackage.DataY = (targetRelative - _relativeCoordinate).Y;
								tempPackage.DataZ = 0;
								tempPackage.Command = 0;
							}
							else if (SM.Z == StepState.ToEnd)//Z到头
							{
								...
								tempPackage.DataX = 0;
								tempPackage.DataY = 0;
								tempPackage.DataZ = (targetRelative - _relativeCoordinate).Z;
								tempPackage.Command = 0;
							}
		...
		private void MonitorOfControlThread()
		{
			while (true)
			{
				_askMonitorTick.WaitOne();
				if (_controlTableThread.ThreadState != ThreadState.Running)
				{
					return;
				}
		...
		{
			_receivedPackage.Add(Convert.ToByte(serialPort1.ReadByte()));

			if(_receivedPackage.Count==19)//如果是19个字节
			{
				if (_receivedPackage[_receivedPackage.Count - 1] == 0x5A &&
					_receivedPackage[_receivedPackage.Count - 2] == 0xA5 &&
					_receivedPackage[0] == 0xAA)//头尾满足要求
				{
					...
					if (tempDataPackage.Command != 0xFF)//表示没有传错
					{
						//获得绝对坐标
		...
		private void progressControlThread()
		{
			LoadingWindow form = new LoadingWindow();
			form.Show();
			...
			form.SetProgressBarValue(20);
			_displayProgress.WaitOne();

			form.SetProgressBarValue(50);
			form.SetText("读取G代码...");
			_displayProgress.WaitOne();

			form.SetProgressBarValue(100);
			form.SetText("即将完成...");
			_displayProgress.WaitOne();
